...
留言板保留当前用户的一句状态，同时把每次“贴上”的内容追加到共享留言历史。服务端最多保留最近 20 条，前端展示最近记录，便于两个人回看小基地动态。
...
目标是允许手机“添加到主屏幕”。当前开发期不主动注册 Service Worker，`index.html` 会注销旧 Service Worker，避免手机继续使用旧 JS/CSS。保留 `service-worker.js` 只作为后续稳定版备用；其中页面导航和 `app.js/styles.css` 都应优先请求网络。

服务器同步依赖网络。浏览器不再把 localStorage 当作数据来源，因此断网时不允许把旧本地状态当成权威数据写回服务器。
...
- 图鉴项目支持点击查看详情。
- 隐藏任务未发现时显示 `???`，不能点开看答案。
- 博物馆点位显示奖杯墙，包括连续训练、挑战完成、双人同日、真实礼物、岛屿建设等成就。
...
- 礼物申请写入服务端 `shared.giftClaims`。
- 对方添加或申请后，即使当前伙伴列表还没完整刷新，码头下方也会直接从共享数据展示待兑现礼物。
- 仍然禁止自己兑现自己的礼物申请。

### 界面修正

- 导出按钮缩小为轻量按钮，避免压住主视觉。
- 头像选择弹窗增加独立尺寸和滚动区域，避免头像网格溢出。
- 底部入口按 6 项布局，避免新增礼物入口后挤压。
- 难度文案从“标准/强化”改成“建设路线/探险路线”，减少与“轻松/标准/挑战”重复。
