...
import { placeRoomFurniture, removeRoomFurniture, ROOM_GRID } from './domain/room';
...
function RoomShell({ state, onLeave, onPlace, onRemove }: {
  ...
  const baseW = gridW + TILE_W * 2.4;
  const baseY = WALL_H + 54;
  const baseH = baseY + (N - 1) * TILE_H + TILE_H * 3.1;
  ...
  const screen = (tx: number, ty: number) => centerC(tx, ty);
  const poly = (pts: { x: number; y: number }[]) => pts.map(p => `${p.x},${p.y}`).join(' ');
  const top = (p: { x: number; y: number }) => ({ x: p.x, y: p.y - WALL_H });
  const lerp = (a: { x: number; y: number }, b: { x: number; y: number }, t: number) => ({ x: a.x + (b.x - a.x) * t, y: a.y + (b.y - a.y) * t });
  const isoDiamond = (x: number, y: number, w = TILE_W, h = TILE_H) =>
    poly([{ x, y: y - h / 2 }, { x: x + w / 2, y }, { x, y: y + h / 2 }, { x: x - w / 2, y }]);

  const P = {
    back: centerC(0, 0),
    right: centerC(N - 1, 0),
    front: centerC(N - 1, N - 1),
    left: centerC(0, N - 1),
  };
  ...
  for (let tx = 0; tx < N; tx += 1) {
    for (let ty = 0; ty < N; ty += 1) {
      const c = centerC(tx, ty);
      tiles.push({ tx, ty, left: c.x - TILE_W / 2, top: c.y - TILE_H / 2 });
    }
  }

  const backWallPts = poly([top(P.left), top(P.back), top(P.right), P.right, P.back, P.left]);
  const leftWallPts = poly([top(P.left), top(P.back), P.back, P.left]);
  const rightWallPts = poly([top(P.back), top(P.right), P.right, P.back]);
  const floorPts = poly([P.back, P.right, P.front, P.left]);

  const wallPt = (fh: number, fv: number) => lerp(lerp(P.back, P.right, fh), lerp(top(P.back), top(P.right), fh), fv);
  const wallPtL = (fh: number, fv: number) => lerp(lerp(P.back, P.left, fh), lerp(top(P.back), top(P.left), fh), fv);
  const stringLights = Array.from({ length: 7 }, (_, i) => lerp(wallPt(0.10, 0.70), wallPt(0.92, 0.68), i / 6));
  ...
  const items = [
    ...
    { kind: 'character' as const, uid: '__char', recipeId: '__char', tx: charPos.tx, ty: charPos.ty, ...screen(charPos.tx, charPos.ty) },
  ...
  const renderFurnitureSvg = (recipeId: string, small = false) => {
    ...
    switch (recipeId) {
      case 'bed':
        return wrap(
          <>
            <ellipse cx="44" cy="75" rx="36" ry="8" fill="rgba(83,52,29,.18)" />
            <path d="M15 42 L45 24 L78 40 L47 59 Z" fill="#fff4d6" />
            <path d="M15 42 L47 59 L47 72 L15 55 Z" fill="#a86643" />
            <path d="M47 59 L78 40 L78 54 L47 72 Z" fill="#8f5638" />
            <path d="M21 39 L45 26 L70 38 L45 52 Z" fill="#fffaf2" />
            <path d="M18 48 L47 31 L76 45 L47 63 Z" fill="#d95f52" />
            <path d="M21 50 L48 64 M32 42 L62 56" stroke="#fff0df" strokeWidth="3" strokeLinecap="round" />
            <path d="M33 38 L33 57 M52 30 L52 52" stroke="#fff0df" strokeWidth="3" strokeLinecap="round" />
            <path d="M25 31 Q33 22 48 28 L38 39 Q29 38 25 31Z" fill="#fff8ea" />
            <rect x="6" y="49" width="14" height="18" rx="3" fill="#8d5c3c" />
            <path d="M6 49 L17 43 L27 48 L20 53 Z" fill="#b98356" />
            <circle cx="16" cy="40" r="4" fill="#66b36a" />
          </>,
          90,
          86,
        );
      ...
      case 'shelf':
        return wrap(
          <>
            <ellipse cx="45" cy="76" rx="32" ry="7" fill="rgba(83,52,29,.18)" />
            <path d="M17 27 L50 12 L76 27 L43 45 Z" fill="#d69d65" />
            <path d="M17 27 L43 45 L43 73 L17 56 Z" fill="#aa6c45" />
            <path d="M43 45 L76 27 L76 56 L43 73 Z" fill="#875438" />
            <path d="M25 35 L43 45 M25 45 L43 55 M25 55 L43 65" stroke="#70452e" strokeWidth="3" strokeLinecap="round" />
            <rect x="30" y="31" width="5" height="15" rx="1" fill="#f4d46a" transform="rotate(27 32 39)" />
            <rect x="38" y="33" width="5" height="16" rx="1" fill="#77b7dc" transform="rotate(27 40 41)" />
            <rect x="47" y="30" width="5" height="18" rx="1" fill="#d95f52" transform="rotate(27 49 39)" />
            <path d="M52 48 L70 39 L70 51 L52 61 Z" fill="#f0ca89" />
            <path d="M56 51 L67 46" stroke="#9d6a42" strokeWidth="2" strokeLinecap="round" />
          </>,
          90,
          86,
        );
      ...
      case 'wooden_table':
        return wrap(
          <>
            <ellipse cx="44" cy="69" rx="31" ry="8" fill="rgba(83,52,29,.18)" />
            <path d="M17 34 L44 19 L71 34 L44 50 Z" fill="#dba46c" />
            <path d="M17 34 L44 50 L44 59 L17 43 Z" fill="#ae7048" />
            <path d="M44 50 L71 34 L71 43 L44 59 Z" fill="#935d3d" />
            <path d="M25 45 L25 63 M61 41 L61 60" stroke="#794b30" strokeWidth="6" strokeLinecap="round" />
            <circle cx="44" cy="33" r="8" fill="#fff2cc" />
            <path d="M38 32 Q44 27 50 32" stroke="#e5b94f" strokeWidth="2" fill="none" strokeLinecap="round" />
          </>,
        );
      ...
      case 'wooden_chair':
        return wrap(
          <>
            <ellipse cx="45" cy="70" rx="26" ry="7" fill="rgba(83,52,29,.18)" />
            <path d="M34 22 L53 12 L67 21 L48 33 Z" fill="#a86d45" />
            <path d="M34 22 L48 33 L48 54 L34 44 Z" fill="#805137" />
            <path d="M24 43 L45 30 L65 41 L44 55 Z" fill="#d8a26b" />
            <path d="M24 43 L44 55 L44 65 L24 52 Z" fill="#a46843" />
            <path d="M44 55 L65 41 L65 50 L44 65 Z" fill="#8c5839" />
            <path d="M31 44 L44 36 L57 43 L44 50 Z" fill="#f3d1a4" />
            <path d="M30 54 L30 68 M59 50 L59 65" stroke="#73472e" strokeWidth="5" strokeLinecap="round" />
          </>,
        );
      ...
      case 'flower_pot':
        return wrap(
          <>
            <ellipse cx="43" cy="70" rx="23" ry="6" fill="rgba(83,52,29,.18)" />
            <path d="M30 46 L56 46 L52 66 L35 66 Z" fill="#c77b48" />
            <ellipse cx="43" cy="46" rx="15" ry="6" fill="#e09b64" />
            <path d="M42 44 C34 31 26 30 23 35 C31 36 37 40 42 44Z" fill="#4f9f5e" />
            <path d="M44 43 C45 29 55 25 61 31 C53 34 49 39 44 43Z" fill="#5cb66d" />
            <path d="M43 44 C39 27 46 20 52 24 C50 32 47 38 43 44Z" fill="#78c87a" />
            <circle cx="31" cy="37" r="3" fill="#ffe07a" />
            <circle cx="56" cy="35" r="3" fill="#ffd0dc" />
          </>,
          88,
          78,
        );
      ...
      case 'lamp':
        return wrap(
          <>
            <ellipse cx="43" cy="71" rx="22" ry="6" fill="rgba(83,52,29,.18)" />
            <path d="M36 62 L51 62 L57 70 L30 70 Z" fill="#9b6c45" />
            <path d="M43 36 L43 63" stroke="#8f623d" strokeWidth="5" strokeLinecap="round" />
            <path d="M26 28 L43 15 L62 28 L56 45 L32 45 Z" fill="#ffe89f" />
            <path d="M26 28 L32 45 L56 45 L62 28" fill="none" stroke="#d5a94f" strokeWidth="3" />
            <circle cx="43" cy="35" r="18" fill="rgba(255,230,145,.34)" />
          </>,
          88,
          78,
        );
      ...
      case 'rug':
        return wrap(
          <>
            <ellipse cx="44" cy="55" rx="36" ry="18" fill="#fffaf0" />
            <ellipse cx="44" cy="55" rx="30" ry="14" fill="#fff" opacity=".9" />
            <ellipse cx="44" cy="55" rx="35" ry="17" fill="none" stroke="#eadfcb" strokeWidth="5" strokeLinecap="round" strokeDasharray="3 6" />
            <circle cx="44" cy="52" r="4" fill="#77bd72" />
      ...
      case 'fence':
        return wrap(
          <>
            <ellipse cx="44" cy="70" rx="33" ry="7" fill="rgba(83,52,29,.18)" />
            <path d="M16 45 L44 29 L72 45" stroke="#9b663f" strokeWidth="8" strokeLinecap="round" fill="none" />
            <path d="M16 56 L44 40 L72 56" stroke="#bd8354" strokeWidth="8" strokeLinecap="round" fill="none" />
            {[20, 34, 48, 62].map((x, i) => (
              <path key={i} d={`M${x} ${38 + i * 3} L${x} ${64 + i * 3}`} stroke="#75482e" strokeWidth="5" strokeLinecap="round" />
            ))}
          </>,
          88,
          78,
      ...
      default:
        return wrap(
          <>
            <ellipse cx="44" cy="70" rx="28" ry="7" fill="rgba(83,52,29,.18)" />
            <path d="M20 42 L44 27 L68 42 L44 58 Z" fill="#d9a36e" />
      ...
      <Card className="island-panel museum-head-card">
        <div className="section-head">
          <div>
            ...
            <p className="museum-sub">方向键 / WASD 移动人物查看。下方选家具，再点地板放置；点家具收回。</p>
          </div>
        </div>
      </Card>

      <Card className="island-panel">
        <div className="room-stage" style={{ width: baseW, height: baseH }}>
          <svg className="room-svg" width={baseW} height={baseH} viewBox={`0 0 ${baseW} ${baseH}`}>
            <defs>
              <linearGradient id="roomWallBack" x1="0" y1="0" x2="1" y2="1">
                <stop offset="0%" stopColor="#fff2df" />
                <stop offset="100%" stopColor="#e9caa2" />
              </linearGradient>
              <linearGradient id="roomWallSide" x1="0" y1="0" x2="1" y2="1">
                <stop offset="0%" stopColor="#f8e4c8" />
                <stop offset="100%" stopColor="#d9b583" />
              </linearGradient>
              <linearGradient id="roomFloorWarm" x1="0" y1="0" x2="0.7" y2="1">
                <stop offset="0%" stopColor="#e5b77a" />
                <stop offset="58%" stopColor="#ca8f54" />
                <stop offset="100%" stopColor="#ad7444" />
              </linearGradient>
              <pattern id="roomWallpaper" patternUnits="userSpaceOnUse" width="24" height="24">
                <circle cx="7" cy="8" r="1.4" fill="rgba(132,170,103,.28)" />
                <path d="M17 15 q3 -4 6 0" stroke="rgba(210,130,110,.23)" strokeWidth="1.4" fill="none" strokeLinecap="round" />
              </pattern>
              <pattern id="roomWoodPlanks" patternUnits="userSpaceOnUse" width="72" height="20" patternTransform="rotate(27)">
                <rect width="72" height="20" fill="rgba(215,151,86,.28)" />
                <path d="M0 19.5 H72 M36 0 V20" stroke="rgba(89,56,27,.22)" strokeWidth="1" />
                <path d="M8 6 C19 2 31 6 43 4" stroke="rgba(255,230,190,.20)" strokeWidth="1" fill="none" />
              </pattern>
              <filter id="roomSoftShadow" x="-20%" y="-20%" width="140%" height="160%">
                <feDropShadow dx="0" dy="5" stdDeviation="5" floodColor="#684325" floodOpacity=".22" />
              </filter>
              <filter id="roomPaper">
                <feTurbulence type="fractalNoise" baseFrequency=".035" numOctaves="2" result="noise" />
                <feDisplacementMap in="SourceGraphic" in2="noise" scale=".75" />
              </filter>
              <radialGradient id="roomWarmGlow" cx="50%" cy="18%" r="78%">
                <stop offset="0%" stopColor="rgba(255,242,196,.45)" />
                <stop offset="100%" stopColor="rgba(255,242,196,0)" />
              </radialGradient>
            </defs>

            <polygon points={backWallPts} fill="url(#roomWallBack)" filter="url(#roomPaper)" />
            <polygon points={leftWallPts} fill="url(#roomWallSide)" opacity=".86" />
            <polygon points={rightWallPts} fill="url(#roomWallBack)" opacity=".94" />
            <polygon points={backWallPts} fill="url(#roomWallpaper)" opacity=".9" />

            <path d={`M${P.left.x},${P.left.y - 9} L${P.back.x},${P.back.y - 9} L${P.right.x},${P.right.y - 9}`} stroke="#8f613a" strokeWidth="11" strokeLinecap="round" fill="none" opacity=".88" />
            <path d={`M${P.left.x},${P.left.y + 2} L${P.back.x},${P.back.y + 2} L${P.right.x},${P.right.y + 2}`} stroke="rgba(77,45,22,.22)" strokeWidth="12" strokeLinecap="round" fill="none" />

            <polygon points={floorPts} fill="url(#roomFloorWarm)" filter="url(#roomSoftShadow)" />
            <polygon points={floorPts} fill="url(#roomWoodPlanks)" opacity=".58" />

            <ellipse cx={originX} cy={baseY + TILE_H * 3.05} rx="122" ry="61" fill="#fff8ef" filter="url(#roomSoftShadow)" />
            <ellipse cx={originX} cy={baseY + TILE_H * 3.05} rx="106" ry="51" fill="#fff" opacity=".88" />
            <ellipse cx={originX} cy={baseY + TILE_H * 3.05} rx="120" ry="60" fill="none" stroke="#eadfcb" strokeWidth="7" strokeLinecap="round" strokeDasharray="4 8" opacity=".92" />

            <g transform={`translate(${originX + 40} ${baseY + TILE_H * 3.02})`}>
              <ellipse cx="0" cy="20" rx="16" ry="5" fill="rgba(84,56,36,.16)" />
              <path d="M-8 5 L10 5 L7 21 L-5 21 Z" fill="#c88453" />
              <ellipse cx="1" cy="5" rx="10" ry="4" fill="#e1a56b" />
              <path d="M0 4 C-11 -4 -12 -12 -6 -15 C-1 -10 1 -4 0 4Z" fill="#63b86b" />
              <path d="M3 4 C8 -8 16 -9 20 -3 C13 0 8 3 3 4Z" fill="#77c87a" />
            </g>

            <g transform={`translate(${originX - 150} ${baseY - WALL_H + 20})`}>
              <rect x="0" y="0" width="58" height="38" rx="7" fill="#b47a49" />
              <rect x="6" y="6" width="46" height="26" rx="5" fill="#fff8e7" />
              <circle cx="19" cy="18" r="6" fill="#f4c15f" />
              <path d="M8 31 L23 20 L34 28 L44 19 L52 31 Z" fill="#82b879" />
            </g>
            <g transform={`translate(${originX + 72} ${baseY - WALL_H + 18})`}>
              <rect x="0" y="0" width="56" height="36" rx="7" fill="#9fc5d8" />
              <rect x="7" y="6" width="42" height="24" rx="5" fill="#fff7e1" />
              <path d="M9 27 C20 12 30 30 47 13" stroke="#d66b5f" strokeWidth="3" fill="none" strokeLinecap="round" />
            </g>
            <g transform={`translate(${originX + 166} ${baseY - WALL_H + 40})`}>
              <circle cx="0" cy="0" r="18" fill="#fff7dc" stroke="#b98558" strokeWidth="5" />
              <path d="M0 0 L0 -9 M0 0 L8 4" stroke="#8b5d3b" strokeWidth="3" strokeLinecap="round" />
              <circle cx="0" cy="0" r="2.5" fill="#8b5d3b" />
            </g>
            <g transform={`translate(${originX - 220} ${baseY - WALL_H + 48})`}>
              <path d="M0 0 Q18 -16 36 0" stroke="#c38c5c" strokeWidth="4" fill="none" strokeLinecap="round" />
              <path d="M8 1 L8 27 M18 -5 L18 23 M28 1 L28 27" stroke="#8fb7c8" strokeWidth="2" />
              <circle cx="8" cy="30" r="4" fill="#f4d36d" />
              <circle cx="18" cy="26" r="4" fill="#d86d62" />
              <circle cx="28" cy="30" r="4" fill="#70b96f" />
            </g>

            <path d={`M${wallPt(0.08, 0.72).x},${wallPt(0.08, 0.72).y} C${originX - 95},${baseY - WALL_H + 110} ${originX + 90},${baseY - WALL_H + 38} ${wallPt(0.94, 0.72).x},${wallPt(0.94, 0.72).y}`} stroke="#b98a57" strokeWidth="3" fill="none" strokeLinecap="round" />
            {stringLights.map((p, i) => (
              <g key={i} transform={`translate(${p.x} ${p.y + (i % 2 ? 9 : -1)})`}>
                <path d="M0 -8 L2 -2 L8 -2 L3 2 L5 8 L0 4 L-5 8 L-3 2 L-8 -2 L-2 -2 Z" fill="#ffd86b" stroke="#e4aa35" strokeWidth="1" />
              </g>
            ))}

            <g transform={`translate(${wallPtL(0.28, 0.26).x - 16} ${wallPtL(0.28, 0.26).y})`}>
              <rect x="0" y="0" width="44" height="30" rx="6" fill="#8b6042" />
              <rect x="5" y="5" width="34" height="20" rx="4" fill="#e6d2ad" />
              <path d="M6 23 C12 12 21 22 28 10 C33 15 36 19 39 23" stroke="#5c7f6a" strokeWidth="3" fill="none" strokeLinecap="round" />
            </g>

            <g transform={`translate(${originX + 230} ${baseY + TILE_H * 1.85}) rotate(-12)`}>
              <ellipse cx="0" cy="29" rx="15" ry="20" fill="#c65b45" />
              <ellipse cx="-2" cy="18" rx="11" ry="14" fill="#d86a50" />
              <circle cx="0" cy="26" r="5" fill="#6e3e2a" />
              <rect x="-3" y="-29" width="6" height="43" rx="3" fill="#8a5634" />
              <path d="M-7 -31 L7 -31 L5 -41 L-5 -41 Z" fill="#8a5634" />
            </g>

            {tiles.map(t => {
              const c = centerC(t.tx, t.ty);
              return <polygon key={`${t.tx}-${t.ty}`} points={isoDiamond(c.x, c.y)} fill="none" stroke="rgba(255,255,255,.13)" strokeWidth="1" />;
            })}
            <ellipse cx={originX + 60} cy={baseY + 88} rx="255" ry="150" fill="url(#roomWarmGlow)" />
          </svg>
          ...
          {tiles.map(t => (
            <button
              key={`t-${t.tx}-${t.ty}`}
              type="button"
              className={`room-tile ${placing ? 'placeable' : ''}`}
              style={{ left: t.left, top: t.top, width: TILE_W, height: TILE_H }}
              onClick={() => { if (placing) { onPlace(placing, t.tx, t.ty); setPlacing(null); } }}
              aria-label={`放置到 ${t.tx},${t.ty}`}
            />
          ))}

          {items.map(item => (
            item.kind === 'character' ? (
              <div key="char" className="room-sprite room-character" style={{ left: item.x, top: item.y + 16, zIndex: 30 + item.tx + item.ty }}>
                <div className="room-shadow" />
                <img className="room-character-img" src={AVATARS.find(a => a.id === state.avatar)?.img || AVATARS[0].img} alt="我" />
              </div>
            ) : (
              <button
                key={item.uid}
                ...
                style={{ left: item.x, top: item.y + 18, zIndex: 10 + item.tx + item.ty }}
                onClick={() => onRemove(item.uid)}
                title="点击收回"
              >
                <div className="room-shadow" />
                {renderFurnitureSvg(item.recipeId)}
              </button>
            )
          ))}
        </div>
      ...
      <Card className="island-panel">
        ...
        <div className="room-palette">
          {palette.length ? palette.map(r => (
            <button key={r.id} type="button" className={`room-palette-item ${placing === r.id ? 'selected' : ''}`} onClick={() => setPlacing(placing === r.id ? null : r.id)}>
              <span className="room-palette-preview">{renderFurnitureSvg(r.id, true)}</span>
              <span>{r.name}</span>
              <small>×{state.inventory[r.id]}</small>
            </button>
          )) : <p className="muted">先去工坊合成家具。</p>}
        </div>
        {placing && <p className="muted" style={{ marginTop: 8 }}>已选中「{GRID_RECIPES.find(r => r.id === placing)?.name}」，点地板放置。</p>}
      </Card>
    </main>
  );
}
