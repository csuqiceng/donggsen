...
describe('博物馆展品', () => {
  it('按 4 房间生成展品：特殊5 + 隐藏6 + 礼物6 + 奖杯8', () => {
    const exhibits = getMuseumExhibits(ctx());
    expect(exhibits.filter(e => e.room === 'special')).toHaveLength(5);
    expect(exhibits.filter(e => e.room === 'hidden')).toHaveLength(6);
    expect(exhibits.filter(e => e.room === 'gift')).toHaveLength(6);
    expect(exhibits.filter(e => e.room === 'trophy')).toHaveLength(8);
  });
...
describe('博物馆房间', () => {
  it('返回 4 房间及各自统计', () => {
    ...
    expect(rooms.find(r => r.id === 'trophy')?.total).toBe(8);
  });
});
