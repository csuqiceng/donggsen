...
describe('buildWeeklyEvent', () => {
  ...
  it('生成周结算事件', () => {
    const event = buildWeeklyEvent({ weekIndex: 0, yearMonth: '2026-06', weeklyCheckins: 8, sameDay: 2, warehouseTotal: 20, weeklyMinutes: 75, insights, now: 1000 });
    ...
    expect(event.summary).toContain('本周运动 75 分钟');
